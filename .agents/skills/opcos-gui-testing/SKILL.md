...
### `propose_plan` is NOT an approval in non-Auto modes (why plans may still never persist)
...
Also: on the unattended path a policy-denied `run_shell` still emits a fake
`shell_process_completed exit 1, duration_ms=null` red row — the phantom-failure shape that the approval
path no longer produces. Check both paths separately.
...
- rendering: `not run · <reason>` on an existing shell row, else a `Not run: <tool>` row. Note both `not
  run` and a genuine non-zero exit use the same `text-danger` colour, so "visually distinct" is only true
  by wording/absence of an exit+duration chip — say so explicitly instead of claiming a colour difference;
...
- `plan_update` with ordinal `step_id` (`"1"`) fails `{"error":"store validation error: plan step not
  found"}`; the model recovers by using `step-<uuid>`. Expect a few wasted calls.
