...
Shell-row metadata gotchas observed on `dbcf337` (all fixed in `b04f35a` — keep the probes, they are the
cheapest regression detectors in the app):
...
- `shell_id_for_session` = `shell-` + first 8 alphanumerics of the session id, and session ids look
  ...
  uniqueness across two sessions, not just presence. Since `b04f35a` it is an FNV-1a hash rendered as
  `shell-[0-9a-f]{8}`.

A cheap, fully decisive probe for all four of those at once — one prompt, two calls:
`sh -c 'exit 3'` and `sh -c 'sleep 1.4; echo slow'`. It gives you (a) a non-zero exit to check against
the red `text-danger` styling, (b) a successful neighbour as a regression guard against a hardcoded
non-zero, and (c) a case where the payload `duration_ms` (~8 ms, only the exec) and the old
timestamp fallback (`completed − started`, ~1.4 s because the two calls share a turn) render as
*different strings*. If the row says `8ms`, the UI is reading the payload; if it says `1.4s`, it is still
on the fallback. Always pick a pair where the two windows disagree — otherwise the test passes either way.

Denied tool calls (see approval gating below) also emit `shell_process_completed`, but with `exit 1`,
no `duration_ms` and `output_trunc = {"error":"tool call denied pending another approval"}`. Exclude
them before asserting "every completion has a duration", and expect them to render as red failed shell
rows even though nothing executed.

When checking the thought-binding fix, run the simulator twice over the *same* store: once with the old
`flush()` (pending thought survives the flush) and once with the fixed one (`flush()` clears
`pendingThought` and resets `thoughtForCallId`). Same events, `LOST=13` vs `LOST=0`, is far stronger
evidence than a single green count — then confirm by counting `Thought for Ns` rows on screen.
...
Before `b04f35a` a newly created session's model selector defaulted to `auto`, which gateways reject
with `Provider request failed`; since `b04f35a` it preselects the first discovered chat-capable model
(`App.tsx` `provider_models` handler). Still read the selector before sending the first prompt — if it
says `auto`, set the model explicitly, and note that the `<select>` may advance one option per click
under automation instead of opening a list. The auto-picked model is whatever the gateway lists first
(e.g. `deepseek-v4-flash`), not necessarily the one you want for a long task.

## Permission modes and approval gating (`Interactive`)

The backend supports Discuss / Plan / Interactive / Auto / Custom (`crates/opcos-policy/src/lib.rs`
`classify`), but the **UI only offers Discuss / "Ask for approval" (interactive) / "Full access" (auto)** —
Plan and Custom are deliberately hidden (`web/src/components/Composer.tsx` `PERMISSION_OPTIONS`, and the
new-session dropdowns in `App.tsx`). To test Plan mode, send the composer slash command **`/mode plan`**
(`src-tauri/src/main.rs` `/mode` → `parse_permission_mode`); the composer chip then reads `Plan` and a
`Mode changed to Plan` notice appears. Do not conclude Plan mode is unreachable just because the picker
lacks it. Previous rounds only ever exercised **Auto**, where nothing is ever asked; use **Interactive**
when the question is "does the harness ask before a side effect". In Interactive, every write and every shell call raises a modal card
...
- Card wording: older builds said *"runs on the bound remote host"* even for local 本机; from the
  approval-sequencing change onward the card should read `runs on <session host name>` (`runs on 本机`
  locally). If it says `runs on the bound host`, the host name is not being plumbed into `ApprovalCard`.
- **Parallel tool calls in one turn used to be denied, not queued** (`{"error":"tool call denied pending
  another approval"}`). After the approval-sequencing change, approving one call replays the batch's
  remaining calls in assistant-message order, so a second approval-requiring call raises its own card.
  Good regression probe: one prompt asking for exactly two `run_shell` calls (`echo alpha`, `echo beta`)
  — approve the first, expect a second card, then two real results in order. Grep the whole event dump
  for `denied pending another approval`; it must be 0.
- Budget wall-clock: each approval is a click plus a model round-trip, so a 5-part task can need ~15
  approvals.

### `propose_plan` routing (fixed at `a545cf8` — verify per build)

At `a545cf8` and later: `propose_plan` is `ToolRisk::Read`, so in **Interactive** it executes inline and
persists immediately (result `{"status":"created","plan_id":…,"steps":N}`, rows in `plans`/`plan_steps`,
`Created N Tasks` in the transcript) while the batch's approval-requiring call raises its own card; only
**Plan** mode parks it, and Plan mode emits `approval_pending` so a real card renders. `resolve_inbox`
now accepts `kind` ∈ {`approval`,`plan`}, so a `visibility='inbox'` plan item resolved from the Inbox
("Plan confirmation" card) also goes through `resolve_approval` → `approval_resolved{approved:true}` and
creates the plan. Assertions that catch a regression here: `plans`/`plan_steps` counts, the
`propose_plan` result string, and **zero** `user_question_answered {"answer":"allow"}` events.
Known remaining gap: the `resolve_approval` approve path emits no `todo_update`, so a Plan-mode approval
persists the plan but shows **no** `Created N Tasks`/progress row in the transcript.

Progress rows: `4/6 #5 <step>` (space present since `a545cf8`) and restated before shell/edit/read/list
rows via `appendPlanProgress` in `timeline.ts` — but when no step is `in_progress` it falls back to the
**last** step, so a fresh plan restates `0/8 #8 …` instead of Devin's `1/4 #1 …`. Check the ordinal, not
just the presence of the row. Per-row wall-clock timestamps are still absent by design.

### Historical: `propose_plan` was NOT an approval in non-Auto modes (pre-`a545cf8`)

`execute_tools` routes `ask_user` **and** `propose_plan` (when mode ≠ Auto) through `save_pending`, not
through the policy `NeedsUser` path. Consequences to check before blaming the model or the approval code:

- the pending row is `kind='plan'`, `state='propose_plan'`, `visibility='inline'`;
- **no `approval_pending` event is emitted**, and the transcript only builds approval cards from
  `approval_pending`/`ask_user_pending` events (`web/src/timeline.ts`), so nothing appears on screen;
- the Inbox lists only `visibility='inbox'` pendings, so it shows "0 pending";
- the turn therefore halts silently with status `Ready`, and every later call of that batch stays parked.

Workaround for reaching the plan card at all: Composer → Mode menu → **Send approvals to Inbox**
(unattended) makes new pendings `visibility='inbox'`. But note `resolve_inbox` branches on
`item.kind == "approval"`; a `kind='plan'` item goes to `resolve_pending_input`, i.e. it is answered as a
*question* (`user_question_answered`, result `{"answer":"allow"}`) and **still creates no plan**. So to
prove plan persistence/plan progress rows today you need **Auto** mode; in Auto, `propose_plan` executes
normally, `plans`/`plan_steps` fill, `todo_update` fires and the transcript shows `Created N Tasks` plus
Devin-style `4/6#5 <step>` rows. Always state which mode the plan evidence came from.

Also: pre-`a545cf8`, on the unattended path a policy-denied `run_shell` emitted a fake
`shell_process_completed exit 1, duration_ms=null` red row — the phantom-failure shape that the approval
path no longer produces. Check both paths separately. Since `a545cf8` policy/preflight denials emit
`tool_call_denied` and no shell completion, driven by an internal `_opcos_not_executed` marker that must
be stripped from persisted and provider-visible results. Deterministic trigger: turn **Send approvals to
Inbox** on (unattended) and ask for one `run_shell` — `decide_with_rules` denies any Write/Execute/External
risk outright (`opcos-policy/src/lib.rs`). Then turn the toggle **off** in the same session and approve
`sh -c 'exit 3'` so the muted `not run` row and a genuine red exit row sit in one screenshot.
When grepping the store for the marker, use `instr(col,'_opcos_')>0` — SQL `LIKE '%_opcos_%'` treats `_`
as a wildcard and returns hundreds of false positives.

### Denied / canceled call shapes (post approval-sequencing)

- denied target → result `{"error":"tool call denied by user"}`, events `approval_resolved{approved:false}`
  + `tool_call_denied{reason:"denied by user"}`;
- the rest of the batch → `{"error":"tool call canceled after approval denial"}` +
  `tool_call_denied{reason:"canceled after approval denial"}`;
- **no `shell_process_completed`** for any of them — assert that count is 0 rather than eyeballing colours;
- rendering: `not run · <reason>` on an existing shell row, else a `Not run: <tool>` row. Pre-`a545cf8`
  both `not run` and a genuine non-zero exit used `text-danger`; since `a545cf8` denied rows are
  `text-muted` (grey) while a real non-zero exit stays red — assert the colour from a zoomed screenshot
  showing both rows in one frame;
- resolved cards render `Approved` / `Declined` with the buttons removed (driven by `approval_resolved`).

### Model behaviour notes worth re-checking each round

- `ask_user` does get used for genuinely unanswerable requirements (Question card with options + free-text
  box, one `ask_user_pending` event). Count the events; a prose question is a different finding.
- `plan_update` with ordinal `step_id` (`"1"`) failed pre-`a545cf8`; since `a545cf8` the store resolves
  ordinals to the durable UUID and an invalid id answers `plan step not found; use an ordinal from 1 to N
  or one of: step-<uuid>, …`. Probe both in one steer: `step_id "1"` then `step_id "99"`, and ask the model
  to quote the error verbatim.
- `plan_update` itself needs approval in Interactive, so expect a card per call.
...
- `RVM_LINUX_URL` / `RVM_LINUX_TOKEN` — working Linux RVM host (workspace `/home/ctyun`).
- `RVM_DEVBOX_URL` / `RVM_DEVBOX_TOKEN` — DevBox (win32) host; the linux token is rejected with 401.
