...
pub trait RvmClient: Send + Sync {
    ...
    async fn computer_use(
        ...
        action
            .validate(bounds)
            .map_err(|error| RvmError::InvalidComputerAction(error.to_string()))?;
...
impl HttpRvmClient {
    ...
    pub fn ide_url(&self, folder: &str) -> Url {
...
mod tests {
    ...
    #[test]
    fn ide_url_contains_only_the_direct_host_route_and_folder() {
        ...
        .unwrap();
        let url = client.ide_url("vscode-remote://linux/workspace");
        assert_eq!(url.path(), "/ide/");
        assert_eq!(
