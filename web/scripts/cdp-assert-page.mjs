...
const result = await command("Runtime.evaluate", {
  expression: `JSON.stringify({ text: document.body?.innerText || "", root: document.getElementById("root")?.innerHTML.length || 0 })`,
  returnByValue: true,
});
const page = JSON.parse(result.result?.result?.value || '{"text":"","root":0}');
...
if (!matched || page.root <= 0) process.exitCode = 1;
socket.close();
