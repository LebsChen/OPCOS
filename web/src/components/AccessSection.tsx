...
const setSessionConnection = async () => undefined;
const subscribeChannel = async () => undefined;
const unsubscribeChannel = async () => undefined;
const ConnectorBadge = () => null;
...
const useRoots = () => ({ roots: [], refresh: () => undefined });
const AddFolderForm = () => null;
const ConnectSetup = () => null;
const RootRow = () => null;
const ChannelPicker = () => null;
...
const BTN_ACCENT =
  "text-[12px] px-2.5 py-1.5 rounded-lg bg-accent text-white shrink-0";
const BTN_BORDERED =
...
export function AccessSection({
  ...
  const channelsOf = (connector: string) =>
    subs.filter(
      (s) => s.session_id === sessionId && platformOf(s.channel) === connector,
    );
  const addChannel = async () => {
    const raw = draft.trim();
    if (!raw || !channelsFor) return;
    const channel =
      raw.includes(":") || raw.startsWith("#") ? raw : `${channelsFor}:${raw}`;
              ...
              <div>
                ...
                <div className="space-y-1">
                  {connected.map((c) => (
                    ...
                    >
                      <ConnectorBadge
                        connector={visualFor(c.connector, "connector", byName)}
                        size={24}
                      />
                  ...
                  <div className="mt-1.5">
                    ...
                    <div className="mt-1 max-h-64 overflow-y-auto">
                      {results.map((c) => (
                        ...
                        >
                          <ConnectorBadge
                            connector={visualFor(c.name, "connector", byName)}
                            size={22}
                          />
                  ...
                  <div className="space-y-1">
                    {recommended.map((r) => (
                      ...
                      >
                        ...
                        <button
                          className={
                            r.tier === "core" ? BTN_ACCENT : BTN_BORDERED
                          }
                          onClick={() => {
...
function ConnectInline({
  ...
  return (
    <div>
      ...
      {c.blurb && (
        <p className="text-[12px] text-muted mb-1 leading-relaxed">{c.blurb}</p>
      )}
      <div className="-mx-2">
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        Connecting makes {c.title} available to all OPCOS sessions — the toggle
        in this list controls just this session.
      </p>
    </div>
  );
}
...
function ChannelsInline({
  ...
  return (
    <div>
      ...
      {error && (
        <p
          className="text-[11px] text-warnInk mt-1.5 leading-snug"
      ...
      <p className="text-[10.5px] text-faint mt-1.5 leading-snug">
        The agent receives messages posted to these channels. Removing one stops
        this session from listening — the connector stays connected.
      </p>
    </div>
  );
}
