...
export function ModelsTab() {
  ...
  if (!settings)
    return (
      <div className="text-[13px] text-muted">{translate("Loading\u2026")}</div>
    );
    ...
    <div>
          ...
          ps.credentialed ? (
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    `Remove the ${info?.title} key from this computer?`,
                  )
                )
                  ps.removeKey();
              }}
      ...
      {ps.sel === "openai" && settings.source === "env" && (
        <p className="text-[12px] text-muted mt-3 leading-relaxed">
          {translate("keySetVia")} <code>OPENAI_API_KEY</code>{" "}
          {translate("keyEnvironment")}
        </p>
      )}

      {info?.configured ? (
        <div className="mt-6">
          <div className={SEC_H + " mb-1.5"}>{translate("Models")}</div>
          <p className="text-[12px] text-muted mb-2.5 leading-relaxed">
            Ticked models show in the composer's picker; the black badge marks
          ...
          <ModelChecklist
            ...
            onChanged={(next) =>
              setSettings((s) =>
                s ? { ...s, models: next.models, model: next.model } : s,
              )
            }
          />
        </div>
        ...
        (info?.suggested_models?.length || 0) > 0 && (
          <div className="mt-6" data-testid="model-preview">
            ...
            <p className="text-[12px] text-muted mb-2.5 leading-relaxed">
              Curated, agent-capable models this provider serves — add your key
...
function ComposerPickerCard({
  ...
  return (
    <div className="mt-6" data-testid="composer-picker">
      <div className={SEC_H + " mb-1.5"}>
        {translate("In the composer's picker")}
      </div>
      <p className="text-[12px] text-muted mb-2.5 leading-relaxed">
            ...
            <div className="mlist-row" key={id}>
              <label className="mlist-main">
                <input
                  ...
                  onChange={() =>
                    removeModel(id).then((r) => r.ok && onChanged())
                  }
                />
              ...
              {isDefault ? (
                <span className="mlist-default">{translate("default")}</span>
              ) : (
                <button
  ...
  return (
    <div className="space-y-3">
      <p className="text-[12.5px] text-muted leading-relaxed">
        External tool servers (stdio or HTTP), shared across all agents. Enabled
        servers' tools are permission-gated. Changes apply to new sessions —{" "}
        <button
      ...
      {MCP_PRESETS.filter((p) => !servers.some((s) => s.name === p.name)).map(
        (p) => (
          <div
            key={p.name}
            className={CARD + " p-3.5 flex items-center gap-3"}
            data-testid={`mcp-preset-${p.name}`}
          >
    ...
    <div className={CARD + " p-3.5"}>
      <div className="flex items-center gap-3">
        <Toggle
          ...
          onChange={onToggle}
          title={translate("Enable this server")}
        />
        <div className="flex-1 min-w-0">
          <div className="text-[14px] font-medium">{server.name}</div>
          <div className="text-[11.5px] text-faint">
            {server.transport} ·{" "}
            {authorizing ? "signing in…" : server.status.replace("_", " ")}
      ...
      {server.last_error && server.status !== "connected" && (
        <div className="text-[12.5px] text-danger mt-1.5">
          {server.last_error}
        </div>
      )}
      {toolErr && (
        <div className="text-[12.5px] text-danger mt-1.5">{toolErr}</div>
      )}
      {tools && (
...
export function UnauthorizedBlock({
  ...
  return (
    <div
      ...
      data-testid={
        teamId ? `unauthorized-${c.name}-${teamId}` : `unauthorized-${c.name}`
      }
    >
          ...
          >
            ...
            <div className="flex items-center gap-1.5 mt-2">
              <button
                ...
                title={translate(
                  "Add the sender to the allow-list and deliver this message now",
                )}
              ...
              <button
                ...
                title={translate(
                  "Add the sender to the allow-list; this message is discarded",
                )}
              ...
              <button
                ...
                title={translate("Throw this message away")}
...
export function ListeningSessionsBlock({ c }: { c: Connector }) {
  ...
  return (
    <div
      className="border-t border-line px-3.5 py-3"
      data-testid={`listening-${c.name}`}
    >
      <div className={SEC_H + " mb-2"}>
        Sessions listening to {c.title} channels · {mine.length}
      </div>
      {mine.length === 0 ? (
        <div className="text-[12px] text-faint">
          None yet — open a session's Sources ▸ Channels to subscribe it to a
          channel.
        </div>
      ) : (
          ...
          {mine.map((s) => (
            ...
            >
              ...
              <button
                ...
                title={translate("Unsubscribe this session")}
                onClick={async () => {
...
export function AllowlistBlock({
  ...
  return (
    <div className="border-t border-line px-3.5 py-3 grid grid-cols-2 gap-5">
      <div>
        <div className={SEC_H + " mb-2"}>{translate("Allowed to message")}</div>
        <div className="flex flex-wrap gap-1.5">
          {allowedUsers.length === 0 && (
            <span className="text-[12px] text-faint">
              nobody yet — Allow a recent sender →
            </span>
          )}
          {allowedUsers.map((u) => (
            <span
      ...
      <div>
        <div className={SEC_H + " mb-2"}>{translate("Recent senders")}</div>
        {unknownRecent.length === 0 ? (
          <div className="text-[12px] text-faint">
            None yet. Message the bot once and it'll show here.
          </div>
        ) : (
          <div className="space-y-1.5">
            {unknownRecent.map((r) => (
...
export function ConnectorTools({
  ...
  return (
    <div className="border-t border-line px-3.5 py-3">
      <div className={SEC_H + " mb-2"}>
        {translate("Tools exposed to OPCOS")}
      </div>
      <div className="space-y-1.5">
        {c.tools.map((tool) => (
          <label
      ...
      {c.managed && !c.mcp && !manualOnly && (
        <div className="space-y-2" data-testid="managed-connect">
          {c.managed_paused ? (
            ...
            <>
              <button
                className={BTN_ACCENT + " opacity-50"}
                disabled
                data-testid="managed-coming-soon"
              >
                {`Connect ${c.title} with one click`}
              ...
              <div className="text-[11.5px] text-faint">
                One-click sign-in is coming soon — connect manually below for
                now:
              </div>
            </>
          ) : cloud?.signed_in ? (
