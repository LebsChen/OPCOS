...
const deleteAutomation = async () => undefined;
const getAutomation = async () => null;
const getAutomations = async () => [];
const markAutomationSeen = async () => undefined;
...
const AutomationQuickstart = () => null;
...
function fromCron(cron?: string | null): { time: string; freq: string } {
  ...
  const [m, h, , , dow] = parts;
  const hh = String(Math.min(23, Math.max(0, parseInt(h, 10) || 9))).padStart(
    2,
    "0",
  );
  const mm = String(Math.min(59, Math.max(0, parseInt(m, 10) || 0))).padStart(
    2,
    "0",
  );
  const freq =
        ...
        <div className="flex flex-col gap-2.5">
          {tasks.map((t) => (
            <div
              className={
                CARD +
                " sched-card px-4 py-3 cursor-pointer hover:border-lineStrong transition-colors"
              }
              key={t.id}
            ...
            >
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {t.enabled ? t.schedule : "Paused"} · next {fmt(t.next_run)} ·{" "}
  ...
  const refresh = () =>
    getAutomation(id)
      .then((d) => {
        ...
        setRuns(d.runs || []);
        setSeenMark((cur) =>
          cur === null ? (d.task?.seen_runs_at ?? 0) : cur,
        );
      })
      .catch(() => {});
  useEffect(() => {
      ...
      <div className="sched-detail">
          ...
          <div className="sched-actions">
            {editing ? (
              ...
              <>
                <button
                  className="btn-primary sm"
                  onClick={() => onRunNow(id, task.title)}
                >
                  ▶ Run now
                </button>
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled ? `Active · next ${fmt(task.next_run)}` : "Paused"} ·{" "}
            {task.schedule}
          </div>
        )}
              ...
              {(task.always_allowed || []).map((rule) => (
                <div className="sched-grant" key={rule.entry}>
                  <span className="sched-grant-rule">
                    <code>{rule.tool}</code>
                    {rule.target && (
                      <span className="sched-grant-target">
                        {" "}
                        → {rule.target}
                      </span>
                    )}
        ...
        {runs.map((r) => (
          ...
          >
            <div className="sched-run-row">
              <span>
                {seenMark !== null && r.started_at > seenMark && (
                  <span className="run-new-pill" data-testid="run-new">
                    new
                  </span>
                )}
                {fmt(r.started_at)} ·{" "}
                <span className={"run-" + r.status}>{r.status}</span> ·{" "}
                {r.trigger}
                {r.artifacts.length > 0 && (
                  <span className="dim"> · {r.artifacts.length} file(s)</span>
                )}
              </span>
            ...
            {r.result_text && (
              <div className="sched-run-peek">{r.result_text}</div>
            )}
