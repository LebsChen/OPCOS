...
const readArtifact = async () => null;
const revealArtifact = async () => undefined;
...
function kindFromPath(path: string): string {
  ...
  if (["py", "js", "ts", "tsx", "jsx", "json", "sh", "css"].includes(ext))
    return "code";
  return "text";
}
...
export function RightRail(props: any) {
  ...
  const reloadSelected = () => {
    ...
    setContent(null);
    return readArtifact(sessionId, selected.path)
      .then(setContent)
      .catch(() => setContent(null));
  };
  ...
  useEffect(() => {
    ...
    const match = (list: ArtifactInfo[], path: string) =>
      list.find(
        (a) =>
          a.path === path || a.path.endsWith("/" + path) || a.name === path,
      );
    ...
    return (
      <div className="rail-todo-list">
        ...
        {running && (
          <div className="rail-muted">
            {toolNames.length
              ? `${toolNames.length} tool call${toolNames.length === 1 ? "" : "s"} so far.`
              : "Working..."}
          </div>
        )}
      </div>
  ...
  if (running) {
    return (
      <div className="rail-muted">
        ...
        {toolNames.length
          ? ` with ${toolNames.length} tool call${toolNames.length === 1 ? "" : "s"} so far.`
          : "."}
      </div>
    );
  }
      ...
      <div className="artifact-preview">
        {!content ? (
          ...
          <div className="artifact-open-prompt">
            ...
            <p>
              This {/\.pptx?$/i.test(artifact.name) ? "PowerPoint" : "Word"}{" "}
      ...
      {(note || body.length > MAX_TABLE_ROWS) && (
        <div className="rail-muted artifact-table-note">
          ...
          {body.length > MAX_TABLE_ROWS
            ? ` Showing first ${MAX_TABLE_ROWS} of ${body.length} rows.`
            : ""}
        </div>
      )}
    </div>
...
function PdfViewer({ dataUrl }: { dataUrl: string }) {
  ...
  useEffect(() => {
    ...
    Promise.reject(new Error("Artifact preview is unavailable in OPCOS"))
      .then(async (pdfjs) => {
  ...
  if (error)
    return (
      <div className="rail-error artifact-table-note">
...
function SheetViewer({ dataUrl }: { dataUrl: string }) {
  ...
  if (error)
    return (
      <div className="rail-error artifact-table-note">
        Could not parse spreadsheet: {error}
      </div>
    );
  if (!sheets)
    return (
