// Unsupported OpenWorker integrations/persona/gallery sections are removed during
...
function VoiceInputSection() {
  ...
  const [phase, setPhase] = useState<
    "idle" | "downloading" | "verifying" | "testing" | "transcribing"
  >("idle");
  ...
  const remove = async () => {
    if (
      !window.confirm("Delete the local Whisper model and disable Voice Input?")
    )
      return;
    setError(null);
    try {
  ...
  const progressPercent = Math.min(
    100,
    Math.round(((progress?.downloaded_bytes || 0) / progressTotal) * 100),
  );
  const ready =
        ...
        <div className="space-y-4">
          ...
          <div className={CARD}>
            ...
            <div className="border-t border-line bg-paper/50 px-4 py-3 grid grid-cols-2 gap-3 text-[12px] text-muted">
              <div>
                <span className="block text-ink font-medium">Mac</span>macOS 12+
                · Apple Silicon M1+
              </div>
              <div>
              ...
              <div>
                <span className="block text-ink font-medium">Processor</span>4
            ...
            {downloading && (
              <div className="border-t border-line px-4 py-3">
                ...
                <div className="mt-1.5 text-[11.5px] text-muted flex">
                  <span>
                    {formatBytes(progress?.downloaded_bytes || 0)} of{" "}
                    {formatBytes(progressTotal)}
                  </span>
...
function CompactionCard() {
  ...
  return (
    <div className={CARD + " p-4 mb-4"} data-testid="compaction-card">
            ...
            onChange={(e) =>
              save({
                compaction_threshold_pct:
                  Math.max(10, Math.min(Number(e.target.value) || 80, 95)) /
                  100,
              })
            }
          />
      ...
      <div className={FIELD_HELP}>
        The cap makes very-large-context models compact early — quality and
        speed degrade well before their nominal limit.
      </div>
