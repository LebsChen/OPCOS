...
- 淘宝开放平台的开发者入驻要求淘宝账号绑定**企业认证**的支付宝账号，个人账户会被直接拒绝，应用上线还需营业执照副本；一人公司基本走不通官方 API［外部来源，见脚注］。闲鱼/拼多多/京东的准入政策未逐个核验，但同类平台大概率类似
- 走不通官方 API 就只能走浏览器自动化，这意味着第一个闭环的大部分精力会消耗在对抗风控上，而**验证不了阶段 1 那套架构对不对**
- 且有账号封禁的实际风险
