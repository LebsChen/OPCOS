...
- **P2-1 Harness 抽象与进程流**：
  ...
  - ACP 通过通用 client 接入；OpenCode 使用 `opencode acp` launch recipe，不再维护独立 harness；
  - 已移除的 OpenCode harness 会话保留为只读 transcript，创建新会话时使用 ACP recipe；
  - OpenCode 的 ACP recipe 复用 ACP 的 Inbox、审批、状态、审计、产物登记和中断恢复路径。
