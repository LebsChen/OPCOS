...
会话在创建时持久化 harness 选择，默认是 `builtin`。Harness 选择在首个 turn 后不可切换：已有 transcript、外部 session ID、pending 请求和状态机不会被另一种运行时语义接管；需要更换 harness 时创建新会话，原会话保持只读历史。桌面端会在创建会话前探测目标 Host 的 `process_stream` 能力和 `opencode` 命令，失败原因直接显示为不可用。
