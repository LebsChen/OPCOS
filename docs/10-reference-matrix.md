...
| PTY/Desktop/Browser | Outpost execution surface［Devin文］       | sandbox surface，细节未确认             | hosted worker 连接［OW文］                          | browser endpoints［OWK码］                        | `/pty-ws`, `/vnc-ws`, `/cdp-ws`［CD码］              | Webview 直连远端 surface URL；协议 capability 仍需统一。                    |
| Web IDE             | Devin environment surface［Devin文］       | 未在抓取文档中确认                      | OpenCode/worker 连接［OW文］                        | 未确认独立 IDE bridge                             | `/ide/*` token gate［CD码］                          | `ide_url` 返回远端 URL 并由 webview 直连；无后端 IDE proxy task。          |
...
| store schema      | [01-data-model.md](01-data-model.md)             | 当前 migration 建立的 43 张表及目标迁移关系。                    |
