...
- pending approval 在 store 里由独立的 `pending(session_id, call_id, tool, arguments, state)` 表保存，并与对应 `tool_calls` 按 `call_id` 合并；前端负责转成审批卡片。
...
3. 否则挂起，写入 `pending` 和对应的 `tool_calls`，发 `approval` 事件
