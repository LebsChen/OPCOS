...
本地自动化触发器只监听回环地址 `127.0.0.1`，并要求随机本地 token；
OPCOS 不提供公网入站 webhook，也不把本机 agent 控制面暴露到互联网。
公网入站 webhook 属于 Cloud 形态 B 的事件 relay，必须等云端 relay、
鉴权、重放保护和生命周期设计完成后再实现。
